...
void J9::RecognizedCallTransformer::process_jdk_internal_util_ArraysSupport_vectorizedMismatch(TR::TreeTop* treetop, TR::Node* node)
   ...
   TR::Node* log2ArrayIndexScaleL = TR::Node::create(node, TR::i2l, 1, log2ArrayIndexScale);

   TR::Node* lengthInBytes = TR::Node::create(node, TR::lshl, 2,
      TR::Node::create(node, TR::i2l, 1, length),
      log2ArrayIndexScaleL);

   TR::Node* mask = TR::Node::create(node, TR::lor, 2,
      TR::Node::create(node, TR::lshl, 2,
         log2ArrayIndexScaleL,
         TR::Node::lconst(node, 1)),
   ...
   TR::Node* mismatchIndex = TR::Node::create(node, TR::arraycmplen, 3);
   // TODO: replace the following aladd's with generateDataAddrLoadTrees when off-heap memory changes come in
   mismatchIndex->setAndIncChild(0, TR::Node::create(node, TR::aladd, 2, a, aOffset));
   mismatchIndex->setAndIncChild(1, TR::Node::create(node, TR::aladd, 2, b, bOffset));
   mismatchIndex->setAndIncChild(2, lengthToCompare);
   mismatchIndex->setSymbolReference(getSymRefTab()->findOrCreateArrayCmpLenSymbol());
   ...
   TR::Node* elseNode = TR::Node::create(node, TR::ishr, 2, mismatchIndex, log2ArrayIndexScale);
   TR::Node* conditionNode = TR::Node::create(node, TR::lcmpeq, 2, mismatchIndex, lengthToCompare);
