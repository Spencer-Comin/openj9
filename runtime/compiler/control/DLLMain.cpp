...
IDATA J9VMDllMain(J9JavaVM* vm, IDATA stage, void * reserved)
   ...
   switch(stage)
      ...
      case ABOUT_TO_BOOTSTRAP:
         ...
         static const bool ignoreAggressiveThroughput = feGetEnv("TR_IgnoreAggressiveThroughputForSetSCC") != NULL;
         if (vm->sharedClassConfig && ((vm->sharedClassConfig->runtimeFlags & aotFlags) == aotFlags) &&
             !(TR::Options::getAggressivityLevel() == OMR::Options::AGGRESSIVE_THROUGHPUT && !ignoreAggressiveThroughput))
            {
