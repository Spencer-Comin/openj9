...
static TR::Register * generate2DArrayWithInlineAllocators(TR::Node *node, TR::CodeGenerator *cg,  int32_t leafArrayElementSize)
   ...
   if (needsAlignHeader)
      {
      generateRegImmInstruction(TR::InstOpCode::ADD8RegImm4, node, spinePtrReg, alignmentInBytes-1, cg);
      generateRegImmInstruction(TR::InstOpCode::AND8RegImm4, node, spinePtrReg, -alignmentInBytes, cg);
      }
   ...
   generateLabelInstruction(TR::InstOpCode::JGE4, node, helperLabel, cg);
   ...
   if (clearAllocation)
      ...
      int sizeShift = trailingZeroes(8 | static_cast<int32_t>(needsAlignHeader ? alignmentInBytes : (contiguousArrayHeaderSize | leafArrayElementSize)));
